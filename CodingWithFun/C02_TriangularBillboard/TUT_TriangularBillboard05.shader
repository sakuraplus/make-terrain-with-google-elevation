...
Shader "TUT/TriangularBillboard05_TypeA" {
	Properties {
	...
	SubShader {
		...
		fixed4 frag(v2f i) : SV_Target {
			...
			_Progress*=ceil(1/_Width)*_Width;
					
				fixed posfloor=floor(_Progress/_Width)*_Width;					
				if(i.uvORI.x<posfloor){
					//grids prev
					color=tex2D(_MainTexB, i.uvTB).rgba;//texB;
				}else if(i.uvORI.x>(posfloor+_Width)){
					//grids next
					color= tex2D(_MainTex, i.uvTA).rgba ;//texA;
				}else{
						...
						fixed2	uvA = fixed2(i.uvTA.x+(posmod-_Width)* posTypeA/(1-posTypeA), i.uvTA.y);;
						fixed2	uvB = fixed2(i.uvTB.x+(1-posTypeA)*posmod /posTypeA, i.uvTB.y);;
