...
Shader "tut/VC-M-3" {
	Properties {
		...
		_OutlineAlphaIn ("Alpha ", Range(0, 1)) = 0
		_OutlineAlphaOut ("Alpha on mask", Range(0, 1)) = 0
	...
	SubShader {
		...
		#include "UnityCG.cginc"
			...
			fixed _OutlineAlphaIn;
			fixed _OutlineAlphaOut;
...
//////////////////////////////			
			half RobertDepthN(v2f i) {
				...
				#endif		
				float4 LD = ((tex2D(_MainTex, i.uvTA[1].xy)));
				float4 RD = ((tex2D(_MainTex, i.uvTA[2].xy)));
				float4 LU = ((tex2D(_MainTex, i.uvTA[3].xy)));
				float4 RU = ((tex2D(_MainTex, i.uvTA[4].xy)));		
...
//				float4 RU = ((tex2D(_CameraDepthNormalsTexture, i.uvTA[4].xy)));			
			...
			fixed4 onmask = lerp(paperOrPaint, texMask, texMask.a);
				...
				fixed alphaLine=lerp(_OutlineAlphaIn,_OutlineAlphaOut,texMask.a);
				fixed ff =	(1-edge)*alphaLine;
