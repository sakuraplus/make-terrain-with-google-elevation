// shader by Li Jiawei
// 2017/02/23
Shader "test/ExplosionTexture" {
	Properties {
		_MainTex("Albedo (RGB)", 2D) = "white" {}
		_ExpTex("Exp tex", 2D) = "white" {}
	...
	SubShader{
		...
		void geo(triangle v2g p[3], inout TriangleStream<g2f> triStream)
		{
			...
			for (int i = 0; i < 3; ++i)
			{
				...
				float3 v_pos=p[i].pos;
				float2 Suv=TRANSFORM_TEX(p[i].tex0, _ExpTex) + frac(float2(1, 0.0) * _Time.y);
				fixed3 color = float3(0, 1, 0);// tex2D(_ExpTex, Suv).rgb;
				v_pos+=color;
