...
Shader "test/lightRunColorTexture" {
		Properties {
...
//			o.uv[1]*=_MaskTex_ST.xy;		 
		...
		fixed4 frag(v2f i) : SV_Target {
			...
			sum.rgb*=_MainAddup;
			//float GradientY=(sin(_Time.y * _ColorSpeed)+1 )/2;//Íù·µ
			float GradientY=frac(_Time.y * _ColorSpeed);//Ñ­»·
			GradientY=floor(GradientY*_ColorNum)/_ColorNum;
			
			fixed4 gradientColor=tex2D(_GradientMap, fixed4(0,GradientY,0.5,0.5)).rgba;
...
//			fixed4 lightcolor	=_EndColor+lightcolorStep*indcolor;
				...
				if(mask.r>0){
					sum.rgb = lerp (sum.rgb, gradientColor.xyz, mask.rrr);
